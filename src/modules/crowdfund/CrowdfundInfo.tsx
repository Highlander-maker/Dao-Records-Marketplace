...
const CrowdfundInfo: FC<CrowdfundInfoProps> = ({
  collection,
  collectionName,
}) => {
  ...
  const price = parseInt(crowdfundState?.state.price.amount || "0");
  ...
  const progress =
    min_tokens_sold > 0 ? Math.floor((total_sold / min_tokens_sold) * 100) : 0;
  ...
  useEffect(() => {
    ...
    const updateDuration = () => {
      ...
      const now = new Date();
      setDuration(dayjs.duration(expires.diff(now)));
    };
    ...
    <Box w="full" data-testid="crowdfund-info">
      <HStack justify="space-between" data-testid="crowdfund-header">
        <Box>
          ...
          <Text fontSize="xs" fontWeight="thin" color="white">
            Collection: <b>{collectionName}</b>
          </Text>
        </Box>
      </HStack>
