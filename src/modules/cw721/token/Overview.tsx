...
const Overview: FC<OverviewProps> = (props) => {
  ...
  if (chainConfig?.blockExplorerAddressPages[0]) {
    explorerUrl += chainConfig?.blockExplorerAddressPages[0].replace(
      "${address}",
      ""
    );
    if (cw721?.address) {
  ...
  return (
    <>
      <Box data-testid="overview">
        ...
        <Text fontWeight="bold" fontSize="xl" mt="8" color="cyan.100">
          Details
        </Text>
        <Box
          mt="4"
        ...
        >
          ...
          >
            <Link href={LINKS.collection(collection.id)}>
            ...
            <Flex justifyContent="flex-end">
              ...
              >
                <Link href={collection.twitter} target={"_blank"}>
                  <TwitterIcon w={5} h={5} color="cyan.100" />
                </Link>
              </Box>
              <Box
                display="inline-block"
          ...
          <Box display="flex" flexDirection="column">
            <Link href={chainConfig?.chainUrl} target="_blank">
              ...
              >
                <Image
                  ...
                  alt={chainConfig?.chainName + " icon"}
                  mr="10px"
                  width="22px"
                />
                <Text color="cyan.100">{chainConfig?.chainName}</Text>
                {chainConfig?.chainType === "testnet" ? (
                  <Text ml="5px" fontSize={"small"} color="cyan.100">
                    (testnet)
                  </Text>
                ) : null}
              </Box>
            ...
            >
              ...
              <Flex justifyContent="space-between">
                ...
                <Text color="cyan.100" fontWeight="light" fontSize="sm">
                  {token?.tokenId}
                </Text>
              </Flex>
              <Divider orientation="horizontal" my="4" />
            ...
            <ButtonGroup mt="2" size="sm" w="full" data-testid="view-buttons">
              ...
              >
                ...
                <Text color="cyan.100">View on Explorer</Text>
              </Button>
              {token?.metadata && (
                <Button
                  onClick={onOpen}
                ...
                >
                  ...
                  <Text color="cyan.100">View Metadata</Text>
                </Button>
              )}
            </ButtonGroup>
          </Box>
        ...
        {token?.metadata?.youtube_url &&
          token?.metadata.youtube_url.length > 10 && (
            <>
              <Text color="cyan.100" fontWeight="bold" fontSize="xl" mt="8">
                Video:
              </Text>
              <Box
                mt="4"
      ...
      <Modal isOpen={isOpen} onClose={onClose} data-testid="metadata-modal">
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>Metadata Object</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
          ...
          <ModalFooter>
            <CopyButton
              text={JSON.stringify(token?.metadata || "{}")}
              variant="ghost"
            >
              Copy
            </CopyButton>
